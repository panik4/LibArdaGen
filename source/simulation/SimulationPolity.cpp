...
std::vector<PolityId> contiguousSuccessorAssignments(
  ...
  for (size_t successor = 0; successor < successorCount; ++successor) {
	const auto seed = provinceIds[(successor * provinceIds.size()) / successorCount];
...
void implode(const Year nextYear, double centuries,
  ...
  const auto initialPolityCount = static_cast<double>(normalized.provinces.size());
  ...
  for (const auto &[polityId, provinceIds] : territories) {
	...
	const auto maturationProgress =
		configuration.successorMaturationYears <= 0
			...
			: std::clamp(static_cast<double>(nextYear - foundedYear) /
							 static_cast<double>(configuration.successorMaturationYears),
						 0.0, 1.0);
	const auto maturationMultiplier =
		configuration.successorFragmentationMultiplier +
		maturationProgress * (1.0 - configuration.successorFragmentationMultiplier);
	const auto smallPolityProgress =
		configuration.smallPolityProtectionSize == 0 ||
			...
			? 0.0
			: std::clamp(static_cast<double>(provinceIds.size() -
											  configuration.smallPolityProtectionSize) /
							 static_cast<double>(configuration.smallPolityProtectionSize),
						 0.0, 1.0);
	const auto smallPolityMultiplier =
		configuration.smallPolityFragmentationMultiplier +
		smallPolityProgress * (1.0 - configuration.smallPolityFragmentationMultiplier);
	const bool decaying =
		strongestScore > 0.0 &&
	...
	if (decaySplit) {
	  ...
	  const auto successorCount = std::min(
		  provinceIds.size(), std::min(configuredMaximum,
	  ...
	  for (size_t index = 0; index < provinceIds.size(); ++index)
		append({nextYear, EventType::TransferProvince, provinceIds[index],
				...
				successors[static_cast<size_t>(assignments[index])], polityId, -1,
				NoReligion, 0.0, 0.0, "successor state partition"});
...
void consolidateRegions(const Year nextYear, const Configuration &configuration,
  ...
  for (const auto &[regionId, provinceIds] : normalized.regions) {
	...
	const auto dominant = std::max_element(
		ownership.begin(), ownership.end(), [](const auto &left, const auto &right) {

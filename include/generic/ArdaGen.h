...
public:
  ...
  template <typename T> void generateStrategicRegions() {
    ...
    if (config.debugLevel > 5) {
      ...
      Fwg::Gfx::Bitmap areaBmp(config.width, config.height, 24);
      ...
      Fwg::Gfx::Png::save(areaBmp,
                          config.mapsPath + "debug//waterLandAreas.png", false);
    }
    ...
    for (auto &landVor : landVoronois) {
      auto superRegion = std::make_shared<T>();
    ...
    superRegions.erase(
        std::remove_if(
            superRegions.begin(), superRegions.end(),
